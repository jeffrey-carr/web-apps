...
func scoreChoices(choices, golden *BracketTree[Bear], depth int) int {
	...
	if golden.Winner.ID != 0 && choices.Winner.ID == golden.Winner.ID {
		score = 8 - (2 * depth)
		fmt.Printf("MATCH! Depth: %d, Bear: %d, Points: %d\n", depth, golden.Winner.ID, score)
	} else if golden.Winner.ID != 0 {
		fmt.Printf("MISMATCH! Depth: %d, Golden: %d, Choices: %d\n", depth, golden.Winner.ID, choices.Winner.ID)
	}

	leftScore := scoreChoices(choices.Left, golden.Left, depth)
	rightScore := scoreChoices(choices.Right, golden.Right, depth)
