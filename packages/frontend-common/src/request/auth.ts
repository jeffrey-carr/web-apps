...
export const backendGetUser = async (
...
export const getUser = async (
  ...
  let response: User;
  try {
    response = await makeRequest(getAuthRouteInfo(environment), { query: { app } }, f);
  } catch (e) {
...
export const logout = async (environment: Environment): Promise<ServerError | undefined> => {
  const appURL = getAppURL(environment, App.Auth);
